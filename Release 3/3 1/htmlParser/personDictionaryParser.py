	...
	def handle_data(self, data):
		...
		if (self.getData == False and self.t100nFound == True):
			if (data[0] == '*'):
				...
				self.text += data.strip()
				self.t100nFound = False
			elif ('Namensvariationen' in data):
...
class DataFormater():
	...
	def makeDict(self):
		...
		'''
		textList = self.text.strip().split('.:._.:.')
		textList = [item for item in textList if item.strip() != '']
	...
	def formatW(self):
		...
		for index, content in enumerate(w):
			...
			if (not (stripContent[0] == u'\u2022') and len(w)== 1):
				...
				elif ('Autor des Vorwortes' in content or 'darunter: Die' in content or 'Diverse Publikationen in:' in content):
					general.append(content.strip())
				else:
